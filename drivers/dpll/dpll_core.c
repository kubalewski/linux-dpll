...
static void
__dpll_pin_deregister(struct dpll_device *dpll, struct dpll_pin *pin)
{
	dpll_xa_ref_pin_del(&dpll->pin_refs, pin);
...
/**
 * dpll_pin_deregister - deregister dpll pin from dpll device
 * @dpll: registered dpll pointer
 * @pin: pointer to a pin
 *
 * Note: It does not free the memory
...
int dpll_pin_deregister(struct dpll_device *dpll, struct dpll_pin *pin)
{
	...
	mutex_lock(&dpll_device_xa_lock);
	mutex_lock(&dpll_pin_xa_lock);
	__dpll_pin_deregister(dpll, pin);
	mutex_unlock(&dpll_pin_xa_lock);
	mutex_unlock(&dpll_device_xa_lock);
...
dpll_deregister:
	xa_for_each(&parent->dpll_refs, i, ref) {
		if (i < stop) {
			mutex_lock(&dpll_device_xa_lock);
			__dpll_pin_deregister(ref->dpll, pin);
			mutex_unlock(&dpll_device_xa_lock);
		}
	}
...
/**
 * dpll_pin_on_pin_deregister - deregister dpll pin from a parent pin
 * @parent: pointer to a parent pin
 * @pin: pointer to a pin
 *
 * Note: It does not free the memory
 */
void dpll_pin_on_pin_deregister(struct dpll_pin *parent, struct dpll_pin *pin)
{
	struct dpll_pin_ref *ref;
	unsigned long i;
...
EXPORT_SYMBOL_GPL(dpll_pin_on_pin_deregister);
